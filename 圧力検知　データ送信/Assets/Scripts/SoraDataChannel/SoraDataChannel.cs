...
public class SoraDataChannel : MonoBehaviour
{
    ...
    public void SendMessage(int[,,] data)
    {
        ...
        var spanData = MemoryMarshal.Cast<int, byte>(dataList.ToArray().AsSpan());
        var spanDataArray = spanData.ToArray();

        Debug.Log("spandataArray" + spanDataArray[1] + spanDataArray[100] + spanDataArray[200]);

        sora.SendMessage(dataChannelLabel, spanData.ToArray());

        ////éÛêM
        //byte[] recvRawData = spanData.ToArray();
        //int[] recvIntData =  MemoryMarshal.Cast<byte, int>(recvRawData.AsSpan()).ToArray();
        //int[,,] recvIntMultiData = new int[3, 10, 10];
    ...
    IEnumerator sendValue()
    {
        while (true)
        {
            ...
            SendMessage(ForceSensor.PressureDistribution);

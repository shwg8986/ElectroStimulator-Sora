...
//graphical attributes
    ...
    void OnReceivedProcess(byte[] data) {
        ...
        // -------- 圧力センサの値を使った処理 --------
        forceSensorIndicator2D.UpdateDisplay(ref SoraDataChannel.pressureDistribution); // Unityの画面上でデータを視覚化するための処理
        hv513.CalcIntensityWithForce(ref SoraDataChannel.pressureDistribution); // 圧力センサのデータを元に、電気刺激の強度を設定する
        forceSensorSerialHandler.WriteByte(PC_ESP32_MEASURE_REQUEST);
        //touched = forceSensorIndicator2D.TouchedSensor(ref SoraDataChannel.pressureDistribution); //島田追加
        pv_sum = forceSensorIndicator2D.TouchedSensor(ref SoraDataChannel.pressureDistribution); //島田追加
        pv_sum_raw = forceSensorIndicator2D.SensorTouched_Raw(ref SoraDataChannel.pressureDistribution); //島田追加
